...
// point one ready model's base template at our custom clump, but only if a player actually wears it (limits ambient bleed onto same-base pedestrians). the base's own clump is saved once so RestoreAll can put it back. new peds of that base clone our clump on creation
void SwapOne(const artwork::ReadyModel& rm, void* /*ctx*/) {
    ...
    // already swapped this base: check the streamer hasn't stolen our clump (independent of whether anyone still wears it, so we always notice the loss)
    auto it = g_swapped.find(rm.baseId);
    if (it != g_swapped.end()) {
        if (*field == it->second.custom) return; // still installed, nothing to do
        // the template no longer holds our clump: the streamer unloaded the base model and destroyed our borrowed clump (field is now null or a fresh real clump). abandon the dangling pointer and reload a fresh one, then re-swap on the next frame
        artwork::OnCustomClumpLost(it->second.custom, /*reload*/ true);
        g_swapped.erase(it);
        return;
    }

    // keep the base ped model resident even before it is worn, so the very first assignment finds a loaded template and renders on the first try (not only after a second trigger)
...
// CNetGame::Process(this) - thiscall, per frame. our main-thread heartbeat: apply any finished async downloads, then reconcile the clump swaps
    ...
    for (auto& kv : g_swapped) {
        ...
        if (*field == kv.second.custom) {
            *field = kv.second.original; // put the game's own clump back
            ...
            // streamer already destroyed our clump: don't clobber the fresh clump, and tell artwork to abandon the dangling pointer so the upcoming free won't double-free
